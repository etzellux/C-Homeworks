...
void main()
{
	...
	for(i=1;i<20;i++)
	{
		for(j=0;j<19;j++)
		{
			if(ogrenciler[j].isim[0]>ogrenciler[j+1].isim[0])
			{
				temp2 = ogrenciler[j];
				ogrenciler[j] = ogrenciler[j+1];
				ogrenciler[j+1] = temp2;
			}
			else if(ogrenciler[j].isim[0] == ogrenciler[j+1].isim[0])
			{
				if(ogrenciler[j].isim[1]>ogrenciler[j+1].isim[1])
				{
					temp2 = ogrenciler[j];
					ogrenciler[j] = ogrenciler[j+1];
					ogrenciler[j+1] = temp2;
	...
	//}
	FILE *fp;
	fp = fopen("uzem_odev_4_a.txt","w");
	for(i=0;i<20;i++)
	{
		fprintf(fp,"%s %d ",ogrenciler[i].isim,ogrenciler[i].no);
	}
	fclose(fp);
	printf("dosyalar isim sirali olarak yazdirildi...");
